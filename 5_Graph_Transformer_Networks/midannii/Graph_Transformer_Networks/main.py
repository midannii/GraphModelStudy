...
#from torchmetrics import F1Score #from torchmetrics.functional import f1_score
...
if __name__ == '__main__':
    ...
    parser.add_argument("--is_gcn", type=bool, default=True)
    ...
    tmp = None
    runs = args.runs
    #f1_score = F1Score(task="multiclass", num_classes=int(num_classes))
    if args.pre_train:
        runs += 1
    ...
    for l in range(runs):
        ...
        for i in range(epochs):
            ...
            if args.dataset == 'PPI':
                ...
                #train_f1 = torch.mean(f1_score(torch.argmax(y_train.detach(),dim=1), train_target)).cpu().numpy()
            ...
            with torch.no_grad():
                ...
                if args.dataset == 'PPI':
                    ...
                    #val_f1 = torch.mean(f1_score(torch.argmax(y_valid,dim=1), valid_target)).cpu().numpy()
                ...
                if args.dataset == 'PPI':
                    ...
                    #test_f1 = torch.mean(f1_score(torch.argmax(y_test,dim=1), test_target)).cpu().numpy()
