...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
        ...
        """
        my_layer = GATLayer(
                num_in_features=256 * 4,  # consequence of concatenation
                num_out_features=121,
                num_of_heads=8,
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, layer_type, concat=True, activation=nn.ELU(),
        ...
        if layer_type == LayerType.IMP1:
            ...
            self.proj_param = nn.Parameter(torch.Tensor(num_of_heads, num_in_features, num_out_features)) # W
        elif layer_type == LayerType.IMP3 or layer_type == LayerType.IMP2:
            # You can treat this one matrix as num_of_heads independent W matrices
            self.linear_proj = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)
        else:
            self.linear_l = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)
            self.linear_r = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)
    ...
    def init_params(self, layer_type):
        ...
        elif layer_type == LayerType.IMP2 or layer_type == LayerType.IMP3:
...
class GATLayerImp4(GATLayer):
    """
        This implementation adds GAT + GATv2 attention mechanism
    ...
    def forward(self, data):
        ...
        in_nodes_features, edge_index = data  # unpack data, data[0] : [2414, 50], data[1] : [2, 54452]
        num_of_nodes = in_nodes_features.shape[0] # 2414
        ...
        source_nodes_features_proj = self.linear_l(in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        target_nodes_features_proj = self.linear_r(in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        ...
        temp_all_scores = self.leakyReLU(source_nodes_features_proj + target_nodes_features_proj)
        ...
        all_scores = (temp_all_scores * self.scoring_fn_source).sum(dim=-1)
        ...
        attentions_per_edge = self.neighborhood_aware_softmax(all_scores, edge_index[self.trg_nodes_dim], num_of_nodes)
...
class GATLayerImp1(GATLayer):
    ...
    def forward(self, data):
        ...
        in_nodes_features, connectivity_mask = data  # unpack data, data[0] : [2414, 50], data[1] : [2, 54452]
        num_of_nodes = in_nodes_features.shape[0] # 2414
        assert connectivity_mask.shape == (num_of_nodes, num_of_nodes), \
            f'Expected connectivity matrix with shape=({num_of_nodes},{num_of_nodes}), got shape={connectivity_mask.shape}.'
        ...
        # We project the input node features into NH independent output features (one for each attention head)
        nodes_features_proj = torch.matmul(in_nodes_features.unsqueeze(0), self.proj_param) # W * h
        ...
        scores_source = torch.bmm(nodes_features_proj, self.scoring_fn_source) # a 계산 (i)
        scores_target = torch.bmm(nodes_features_proj, self.scoring_fn_target) # a 계산 (j)
