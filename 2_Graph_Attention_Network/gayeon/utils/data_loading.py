...
def load_graph_data(training_config, device):
    ...
    if dataset_name == DatasetType.CORA.name.lower():  # Cora citation network
        ...
        edge_num=[len(value) for value in adjacency_list_dict.values()]
        print('노드 개수:', num_of_nodes)
        print('엣지 개수:', sum(edge_num))
        #노드 개수: 2708
        #엣지 개수: 10858
        if layer_type == LayerType.IMP3: #pytorch geometric 버전
            ...
            #topology: source ids, target ids가 stack되어 있는 형태, shape=[2, 13264]


        elif layer_type == LayerType.IMP2 or layer_type == LayerType.IMP1: #IMP1: transductive setting에 적합, #IMP2: official gat impplement
        ...
        node_features = torch.tensor(node_features_csr.todense(), device=device) #원래 형태대로 변환

        # Indices that help us extract nodes that belong to the train/val and test splits
        train_indices = torch.arange(CORA_TRAIN_RANGE[0], CORA_TRAIN_RANGE[1], dtype=torch.long, device=device) #[0, 140]
        val_indices = torch.arange(CORA_VAL_RANGE[0], CORA_VAL_RANGE[1], dtype=torch.long, device=device) #[140, 140+500]
        test_indices = torch.arange(CORA_TEST_RANGE[0], CORA_TEST_RANGE[1], dtype=torch.long, device=device) #[1708, 1708+1000]
...
def build_edge_index(adjacency_list_dict, num_of_nodes, add_self_edges=True):
    ...
    if add_self_edges:
        source_nodes_ids.extend(np.arange(num_of_nodes))#np.arange=range와 비슷함 
        target_nodes_ids.extend(np.arange(num_of_nodes))

    # shape = (2, E), where E is the number of edges in the graph
    edge_index = np.row_stack((source_nodes_ids, target_nodes_ids)) #shape=[2,E+node 개수]만큼 되어야 하지 않ㄴ나
