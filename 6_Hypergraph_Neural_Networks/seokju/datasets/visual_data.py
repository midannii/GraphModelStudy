...
def load_feature_construct_H(data_dir,
    ...
    """
    # init feature
    print(is_facebook,flush=True)
    # if use_mvcnn_feature or use_mvcnn_feature_for_structure and False:
    #     mvcnn_ft, lbls, idx_train, idx_test = load_ft(data_dir, feature_name='MVCNN')
    # if use_gvcnn_feature or use_gvcnn_feature_for_structure and False:
    #     gvcnn_ft, lbls, idx_train, idx_test = load_ft(data_dir, feature_name='GVCNN')
    print(f'data_dir:{data_dir}')
    if is_facebook:
        face_ft,lbls,idx_train,idx_test = load_ft(data_dir,feature_name='facebook')
    ...
    fts = None
    # if use_mvcnn_feature and False:
    #     fts = hgut.feature_concat(fts, mvcnn_ft)
    # if use_gvcnn_feature and False:
    #     fts = hgut.feature_concat(fts, gvcnn_ft)
    if is_facebook:
        fts = hgut.feature_concat(fts,face_ft)
    if fts is None:
    ...
    H = None
    if use_mvcnn_feature_for_structure and False:
    ...
    if use_gvcnn_feature_for_structure and False:
        tmp = hgut.construct_H_with_KNN(gvcnn_ft, K_neigs=K_neigs,
                                        split_diff_scale=split_diff_scale,
                                        is_probH=is_probH, m_prob=m_prob)
        H = hgut.hyperedge_concat(H, tmp)
    if is_facebook:
        tmp = hgut.construct_H_with_KNN(face_ft,K_neigs=K_neigs,
                                        split_diff_scale=split_diff_scale,
                                        is_probH=is_probH, m_prob=m_prob)
        H = hgut.hyperedge_concat(H, tmp)
    # if H is None:
