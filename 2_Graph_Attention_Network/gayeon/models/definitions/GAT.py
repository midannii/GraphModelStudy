...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
        ...
        for i in range(num_of_layers): #2개
            layer = GATLayer(
                num_in_features=num_features_per_layer[i] * num_heads_per_layer[i],  # consequence of concatenation
                num_out_features=num_features_per_layer[i+1], #[1433, 8 7]
...
class GATLayerImp2(GATLayer):
    ...
    def forward(self, data): #data:(node_features, edge_index)
        #
        ...
        #in_nodes_features: [2708, 1433]
        #connectivity_mask: [2708, 2708]
        ...
        # We project the input node features into NH independent output features (one for each attention head)
        nodes_features_proj = self.linear_proj(in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        print('feat',nodes_features_proj.shape)
        #in_node_features:[2708, 8, 8]

        nodes_features_proj = self.dropout(nodes_features_proj)  # in the official GAT imp they did dropout here as well
        print('feat2',nodes_features_proj.shape)
        #
        ...
        # Optimization note: torch.sum() is as performant as .sum() in my experiments
        print('score', self.scoring_fn_source.size(), self.scoring_fn_target.size())

        #scoring_fn_source, target: [1, 8, 8]
        #scores_source, target: [2708, 8, 1]
        ...
        #scores_source, target: [8, 2708, 1], [8, 1, 2708]
        ...
        all_scores = self.leakyReLU(scores_source + scores_target) #[2708, 8, 8]
        ...
        #print(all_scores.size(), connectivity_mask.size())
        all_attention_coefficients = self.softmax(all_scores + connectivity_mask)
        print(all_attention_coefficients.size())
        #
