...
def load_ppi_data(dataset='PPI'):
    ...
    l_train = len(features)
    for d in valid_data: 
        features.append(d.ndata['feat'])
        labels.append(d.ndata['label'])
    l_valid = len(features)
    for d in test_data: 
        features.append(d.ndata['feat'])
        labels.append(d.ndata['label'])
    l_test = len(features)
    ...
    for d in train_data: 
        ...
        for j in range(len(edges1)): edges.append([edges1[j], edges2[j]])
    for d in valid_data: 
        edges1, edges2 = d.edges()
        for j in range(len(edges1)): edges.append([edges1[j], edges2[j]])
    for d in test_data: 
    ...
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])), 
                        shape=(labels.shape[0], labels.shape[0]), dtype=np.float32) # shape: (3020, 3020)
