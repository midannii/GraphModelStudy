...
def load_link_prediction_data(name, use_test_set=False, limit=None):
    ...
    elif name.lower() == 'tb-toutanova':
        train_file = locate_file('data/tb-toutanova/train.txt')
        val_file = locate_file('data/tb-toutanova/valid.txt')
        test_file = locate_file('data/tb-toutanova/test.txt')
    else:
        raise ValueError(f'Could not find \'{name}\' dataset')
